...
def _Init():
    ...
    if is_win:
        if is_x86:
            dll_path = os.path.join(current_dir, "MVCAMSDK.dll")
            _sdk = windll.LoadLibrary(dll_path)  # Load 32-bit DLL
        else:
            dll_path = os.path.join(current_dir, "MVCAMSDK_X64.dll")
            _sdk = windll.LoadLibrary(dll_path)  # Load 64-bit DLL
